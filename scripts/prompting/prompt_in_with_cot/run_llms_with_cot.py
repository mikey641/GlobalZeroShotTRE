...
from scripts.prompting.prompt_in_with_cot.prompts_for_cot import task_description_6res_only_global, \
    task_description_4res_only_global
...
def get_complete_prompt(data, instructions_func, gen_pairs, cot):
    cot = "Here is the timeline of the events in the document:\n" + cot
...
def main(test_folder, llm_to_use, instructions_func, output_file,
         number_of_pred, selected_file, gen_pairs=True, cot_file=None):
    # load json file
    predictions = dict()
    count = 0
    for i, file1 in enumerate(tqdm(os.listdir(test_folder))):
        ...
        task_desc = get_complete_prompt(data, instructions_func, gen_pairs, cot_file[file1]['target'])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cot_file", help="The cot file path", type=str, required=True)
    ...
    _cot = open_input_file(args.cot_file)
    ...
    if args.test_db in ["nt", "tbd"]:
        _instructions = task_description_6res_only_global
    elif args.test_db in ["matres", "omni"]:
    ...
    for i in range(num_of_repetitions):
        ...
        main(test_folder=_test_folder, llm_to_use=_llm_to_use, instructions_func=_instructions,
             output_file=_output_file, selected_file=_selected_file, number_of_pred=_num_of_pred, cot_file=_cot)
        end_time = time.time()
