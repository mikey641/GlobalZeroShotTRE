...
class TLINK:
    def __init__(self, eventInstanceID, relatedToEvent, relType):
        self.firstEvent = eventInstanceID.replace('ei', 'e')
        self.secondEvent = relatedToEvent.replace('ei', 'e')
...
def extract_tlinks(root):
    ...
    for tlink_element in root.findall('TLINK'):
        ...
        if eventInstanceID and relatedToEventInstance and relType:
            tlinks.append(TLINK(eventInstanceID, relatedToEventInstance, relType))
...
def read_file(file_path):
    ...
    print(f"num of tlinks: {len(tlinks)}")

    return doc, tokens, events, tlinks


def extract_pair(axis_id, tlink):
    pair = dict()
    pair['_pairId'] = str(uuid.uuid4())
    pair['_axisId'] = axis_id
    pair['_firstId'] = tlink.firstEvent
    pair['_secondId'] = tlink.secondEvent
    if tlink.relType == 'SIMULTANEOUS':
...
def read_folder(folder_path):
    ...
    for i, entry in enumerate(os.scandir(folder_path)):
        try:
            doc, tokens, events, tlinks = read_file(entry)
            all_read_files[entry.name] = (doc, tokens, events, tlinks)
